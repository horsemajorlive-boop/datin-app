...
// ---------- Админ-панель: список всех анкет и точечные действия ----------

// Список пользователей с поиском (по имени или по id) и пагинацией —
// «Все анкеты» в админ-панели. Работает и с теми, кто ещё без анкеты.
export function adminListProfiles({ search = '', limit = 30, offset = 0 } = {}) {
  // node:sqlite требует, чтобы в объекте параметров были ТОЛЬКО те имена,
  // что реально встречаются в SQL-тексте конкретного запроса — поэтому
  // держим отдельно параметры WHERE (общие для count и списка) и лимиты
  // (нужны только списку).
  const whereParams = {};
  const where = [];
  ...
  if (q) {
    if (/^\d+$/.test(q)) {
      where.push('u.id = :id');
      whereParams.id = Number(q);
    } else {
      // lower_ru — регистронезависимо и для кириллицы тоже (см. db.js).
      where.push('lower_ru(p.name) LIKE lower_ru(:q)');
      whereParams.q = `%${q}%`;
    }
  }
  ...
  const rows = db
    .prepare(
      `SELECT u.id, u.created_at, u.verified_at, u.is_admin,
              p.name, p.age, p.city, p.is_visible
         FROM users u
  ...
  const openReportsStmt = db.prepare(
    `SELECT COUNT(*) AS n FROM reports WHERE reported_id = ? AND status = 'open'`
  );
  ...
  return {
    ...
    items: rows.map((r) => ({
      ...
      photo: photoStmt.get(r.id)?.url || null,
      verified: r.verified_at != null,
      isVisible: r.is_visible == null ? true : !!r.is_visible,
      isAdmin: r.is_admin === 1,
      createdAt: r.created_at,
      openReports: openReportsStmt.get(r.id).n,
    })),
  };
}

// Показать / скрыть анкету из поиска — то же самое, что hideProfile,
// но в обе стороны и явным флагом (для переключателя в админке).
export function adminSetVisibility(userId, visible) {
  db.prepare(`UPDATE profiles SET is_visible = ? WHERE user_id = ?`).run(
    visible ? 1 : 0,
    userId
  );
  return { ok: true };
...
// Выдать/снять золотую галочку напрямую, без заявки на верификацию.
export function adminSetVerified(userId, verified) {
  if (verified) {
    db.prepare(`UPDATE users SET verified_at = ? WHERE id = ?`).run(now(), userId);
  } else {
...
// Мэтчи любого пользователя — для просмотра переписок админом.
// Формат тот же, что видит сам пользователь (getMatches).
export function adminGetUserMatches(userId) {
  return getMatches(userId);
}

// Сообщения мэтча для админ-просмотра — без проверки участия (админ не
// сторона переписки), sender_id отдаём как есть, экран сам решит, как
// подписать каждую реплику.
export function adminGetMessages(matchId) {
  const rows = db
    .prepare(
      `SELECT id, sender_id, type, text, photo_url, reaction, created_at
         FROM messages WHERE match_id = ? ORDER BY created_at`
