...
Встроенный тест-раннер Node (`node --test`, ничего дополнительно ставить не
нужно) — тесты лежат в `backend/test/`. Работают на изолированной временной
базе (см. `DB_PATH` в тестовых файлах), реальную dev-базу не трогают.
Покрыта самая рискованная логика: подпись Telegram `initData`, свайпы/мэтчи,
дневные лимиты, Premium, буст, разматчивание, маскировка чужих данных без
Premium. На каждый push/PR в `main` те же тесты (плюс линт и сборка
фронтенда) гоняет GitHub Actions — см. `.github/workflows/ci.yml`.
