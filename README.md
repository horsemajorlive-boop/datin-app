...
```bash
cd backend && npm test    # логика сервера
cd frontend && npm test   # утилиты фронтенда
```

Backend: встроенный тест-раннер Node (`node --test`, ничего дополнительно
ставить не нужно) — тесты лежат в `backend/test/`. Работают на изолированной
временной базе (см. `DB_PATH` в тестовых файлах), реальную dev-базу не
трогают. Покрыта самая рискованная логика: подпись Telegram `initData`,
свайпы/мэтчи, дневные лимиты, Premium, буст, разматчивание, маскировка
чужих данных без Premium.

Frontend: [Vitest](https://vitest.dev/) — тесты лежат рядом с проверяемым
кодом (`*.test.js`). Покрыты чистые утилиты: фильтры ленты, форматирование
дат/расстояний, сила анкеты, нормализация профиля/сообщений из API.

На каждый push/PR в `main` те же тесты (плюс линт и сборка фронтенда)
гоняет GitHub Actions — см. `.github/workflows/ci.yml`.
